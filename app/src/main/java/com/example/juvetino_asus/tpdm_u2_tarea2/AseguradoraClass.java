...
public class AseguradoraClass {
        ...
        public AseguradoraClass(Activity activity){
            base = new BaseProyectoAseguradora(activity, "ASEGURADORA",null,1);
        }

        public AseguradoraClass(int idS, String descripcionS, String fechaS, String tipoS, String telefonoS){
            this.idSeguro = idS;
        ...
        public boolean insertar(AseguradoraClass seguro) {
            try {
        ...
        public boolean eliminar(AseguradoraClass seguro) {
            try {
        ...
        public boolean actualizar(AseguradoraClass seguro){
            try{
        ...
        public AseguradoraClass[] consultar(){
            try{
                SQLiteDatabase db=base.getReadableDatabase();
                Cursor c=db.rawQuery("SELECT * FROM SEGURO",null);
                if (c.moveToFirst()) {
                    AseguradoraClass[] seguros = new AseguradoraClass[c.getCount()];
                    int posicion = 0;
                    do {
                        seguros[posicion] = new AseguradoraClass(c.getInt(0),c.getString(1),c.getString(2),c.getString(3),c.getString(4));
                        posicion++;
                    }//do
                    while (c.moveToNext());
                    return seguros;
